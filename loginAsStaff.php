<?php 
	...
	if((isset($_POST['email'])&& isset($_POST['password'])) && (!empty($_POST['email'])&& !empty($_POST['password']))){
		...
		$DB = new Database();
		$validation = $DB->loginAsStaff($Email, $Password);
		if($validation == "error"){
			echo "<script>alert('Database Connection Failed')</script>";
            echo "<script>window.open('loginAsStaff.php' , '_self')</script>";
		}elseif ($validation == "invalid"){
			echo "<script>alert('Email or Password is invalid. Please try again.')</script>";
			echo "<script>window.open('loginAsStaff.php' , '_self')</script>";
		}else{
			session_start();
			$_SESSION["StaffID"] = $validation;
		...
		<div class="row">
			<div class="col-md-5 col-md-offset-5">
				<div class="panel panel-default">
					<a class="btn btn-info" href="loginAsAdmin.php">Back</a><br/><br/>
					<div class="panel-heading">
						<h3 class="panel-title">Login as Staff</h3><br/>
					</div>
					<div class="panel-body">
						<form accept-charset="UTF-8" role="form" action="LoginAsStaff.php" method="post">
							<fieldset>
								...
								<div class="checkbox">
									<label>
										<input type="checkbox" onclick="show_password()"><strong>&nbsp Show Password</strong>
									</label>
								</div>
